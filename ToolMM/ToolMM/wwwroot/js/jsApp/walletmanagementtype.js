...
function fnLoadSuccess(rs) {
    for (var i = 0; i < rs.data.length; i++) {
        var input = {
            'PrivateKey': rs.data[i].privateKey,
            'AddressWallet': rs.data[i].address,
        }
        callApi_userservice(
            apiConfig.api.wallet.controller,
...
function fnSearchSuccess(rspn) {
    if (rspn !== undefined && rspn !== null && rspn.data.length > 0) {
        ...
        var htmlCout = '<div class="col-xl-3 col-lg-3 col-md-3 col-3">' +
            '<label for="walletlable1" class="col-sm-12 col-form-label" style="color: blue">' + "Tổng số ví: " + rspn.count + '</label>' +
            '</div>' +
            '<div class="col-xl-3 col-lg-3 col-md-3 col-3">' +
            '<label for="walletlable1" class="col-sm-12 col-form-label" style="color: red">' + "Ví đã chọn: " + rspn.countSelected + '</label>' +
            '</div>';
